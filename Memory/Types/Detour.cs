...
public class Detour : MemoryObject
{
    ...
    public Detour(string address, byte[] ogBytes, nuint detourAddress, int replaceCount, Mem.DetourType detourType = Mem.DetourType.Unspecified, string signature = "", int signatureOffset = 0, Mem m = null) : base(address, "", m)
    {
        _originalBytes = ogBytes;
        _signature = signature;
        _signatureOffset = signatureOffset;
